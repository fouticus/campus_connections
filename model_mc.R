...
# Dyad independent model with 2 blocks AND accounting for Condition vs. No Condition, and also whether sender/receiver are same gender/mentor family/dyad relationship
N <- 500
k <- 1
results <- list(length=length(semesters)*length(nights))
empirical <- list(length=length(semesters)*length(nights))
