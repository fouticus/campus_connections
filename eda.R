...
# gender breakdown
p_staff_gender <- ggplot(staff, aes(x=night1)) + geom_bar(aes(fill=gender)) + facet_grid(semester ~ room) + theme(legend.position="bottom") + scale_fill_manual(values=c("grey", "pink", "cyan"))
...
# role breakdown
p_staff_role <- ggplot(staff, aes(x=night1)) + geom_bar(aes(fill=role1)) + facet_grid(semester ~ room) + theme(legend.position="bottom") 
