...
roles <- participants[,c("final_id", "role")]
valid_edges <- merge(valid_edges, roles, by.x="sender_final_id", by.y="final_id")
colnames(valid_edges)[length(colnames(valid_edges))] <- "sender_role"
valid_edges <- merge(valid_edges, roles, by.x="receiver_final_id", by.y="final_id")
